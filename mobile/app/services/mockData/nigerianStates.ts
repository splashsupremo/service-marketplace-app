/**
 * Nigerian States & Local Government Areas (LGAs)
 *
 * Complete list of all 36 states plus the Federal Capital Territory (FCT),
 * each with their official Local Government Areas.
 *
 * This is the primary location dataset for the app's State/LGA filters
 * (StateSelector, and any future LGA-level filtering).
 *
 * `NIGERIAN_STATES` (flat list of state names, with "All States" first)
 * is derived from this data, so there is a single source of truth.
 */

export interface StateWithLGAs {
  /** State name, e.g. "Lagos" */
  name: string;
  /** All official Local Government Areas within this state */
  lgas: string[];
}
...
export const NIGERIA_STATES_WITH_LGAS: StateWithLGAs[] = [
  ...
  },
  {
    ...
    lgas: [
      ...
      'Emure', 'Gbonyin', 'Idu0-Osi', 'Ijero', 'Ikere', 'Ikole', 'Ilejemeje',
  ...
  },
  {
    ...
    lgas: [
      ...
      'Kafin Hausa', 'Kazaure', 'Kiri Kasama', 'Kiyawa', 'Maigatari',
  ...
  },
  {
    ...
    lgas: [
      ...
      'Boripe', 'Ede North', 'Ede South', 'Ife Central', 'Ife East',
  ...
  },
  {
    name: 'Plateau',
    lgas: [
      'Bakura', 'Barkin Ladi', 'Bassa', 'Bokkos', 'Jos East', 'Jos North',
...
/**
 * Flat list of state names, derived from NIGERIA_STATES_WITH_LGAS,
 * with "All States" prepended as the default/unfiltered option.
 *
 * Use this anywhere only a state name list is needed (e.g. a simple
 * state filter dropdown without LGA drill-down).
 */
export const NIGERIAN_STATES: string[] = [
  'All States',
  ...NIGERIA_STATES_WITH_LGAS.map((s) => s.name),
];

/**
 * Helper to get the LGAs for a given state name.
 * Returns an empty array if the state isn't found (e.g. "All States").
 *
 * Usage:
 *   const lgas = getLGAsForState('Lagos'); // ['Agege', 'Ajeromi-Ifelodun', ...]
 */
export function getLGAsForState(stateName: string): string[] {
  const match = NIGERIA_STATES_WITH_LGAS.find((s) => s.name === stateName);
